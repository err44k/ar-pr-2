...
export function MapPage(){
                                ...
                                <div className="flex items-center justify-between gap-4">
                                    <div>
                                        ...
                                        <button type="button" onClick={() => {
                                            setDraftCoordinated([])
                                            setDrawRevision(revision => revision + 1);
                                        }} className="flex h-9 items-center gap-2 rounded-md border border-neutral-300 px-3 text-xs font-medium">{draftCoordinates.length > 0 ? <ArrowsClockwiseIcon size={32}/> : <PentagonIcon size={15}/>} {draftCoordinates.length > 0 ? "Перерисовать" : "Рисовать"}</button>
                                    </div>
                            ...
                            <div className="mt-6 border-t border-neutral-200 pt-5">
                                ...
                                <ol className="mt-3 space-y-2 font-mono text-xs text-neutral-600">
                                    {selectedZone.coordinates.map(([lon, lat],index)=>(
                                        <li className="flex items-center justy-between gap-3 border border-neutral-100 pb-2">
    ...
    <button 
        className={`w-fit absolute z-20 right-0 flex h-12 items-center gap-2 rounded-md bg-emerald-700 px-4 text-sm font-semibold text-white ${
        panelOpen
